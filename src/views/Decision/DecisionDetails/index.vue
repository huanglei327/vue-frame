...
import { getDecisionMaking, getQuizInfo, notDecisionMaking, addQuizInfo } from './api'
export default {
    data() {
        return {
            msg: 'asdd',
    ...
    mounted() {
        ...
        getDecisionMaking(that.$route.query.decisionId,that.$common.getUserInfo("userName")).then(callback)
    ...
    methods: {
        getTiwenList() {
            ...
            getQuizInfo(that.$route.query.decisionId).then(callbackA)
        },
        goTiWen() {
            this.show = true
            this.twshow = true
