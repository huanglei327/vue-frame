            ...
            <div>
                <div style="width:100%;overflow:hidden;">
                    <ul class="up-img-ul">
                        ...
                        <li v-for="(item,index) in imglist" :key="index" @click="imgPreview(item)">
                            <img width="70px" height="90px" :src="item">
                        </li>
...
<script>
import { getName, SaveJC } from './api'
    ...
    methods: {
        btnGoJC() {
            ...
            const DecisionMaking = {
                //id: that.$common.MathRand(5),
                decisionName: that.status.jcName,
                decisionType: that.status.infoId,
                decisionContent: that.status.jcContent,
                decisionProposer: that.status.tuserName+',管理员',
                decisionAssessor: that.status.cuserName+',管理员',
